...
export default function TextForm(props) {
  ...
  return (
    ...
    <div className={`container text-${(props.mode==='light')?'dark':'light'} my-3`}>
      ...
      <button disabled={text.length===0} className="btn btn-danger" onClick={changeUpperCase}>Convert to Uppercase</button>
      <button disabled={text.length===0} className="btn btn-danger mx-2" onClick={changeLowerCase}>Convert to Lowercase</button>
      <button disabled={text.length===0} className="btn btn-danger mx-2" onClick={clearText}>Clear Text</button>
      <button disabled={text.length===0} className="btn btn-danger mx-2" onClick={copyText}>Copy Text</button>
      <button disabled={text.length===0} className="btn btn-danger mx-2" onClick={removeExtraSpace}>Remove Extra Spaces</button>
    </div>
